...
integrator1 = IntegralEvaluator(HuzinagaOverlapEngine(), None, None)
integrator2 = IntegralEvaluator(HellsingOverlapEngine(True), None, None)
...
for i,c1 in enumerate(cgfs):
    for j,c2 in enumerate(cgfs):
        S1[i,j] = integrator1.kinetic(c1, c2)
...
for i,c1 in enumerate(cgfs):
    for j,c2 in enumerate(cgfs):
        S2[i,j] = integrator2.kinetic(c1, c2)
end = time.perf_counter()
